...
export function useKeyboardControls(
  ...
  { panSpeed = 0.15, orbitSpeed = 0.03, zoomSpeed = 0.3, onInteract }: {
    panSpeed?: number; orbitSpeed?: number; zoomSpeed?: number; onInteract?: () => void
  } = {}
) {
  ...
  useEffect(() => {
    ...
    const onMouseEnter = () => { canvasHovered.current = true }
    const onMouseLeave = () => { canvasHovered.current = false }
  ...
  useFrame(({ camera }) => {
    ...
    // WASD — pan
    if (keys.current.has('a')) pan.add(_right.current.clone().multiplyScalar(-ps))
    if (keys.current.has('d')) pan.add(_right.current.clone().multiplyScalar(ps))
    if (keys.current.has('w')) pan.add(_forward.current.clone().multiplyScalar(-ps))
    if (keys.current.has('s')) pan.add(_forward.current.clone().multiplyScalar(ps))
